...
class checkpoint():
    ...
    def save_patches_results(self, filename, save_list, scale, size, index):
        filename = '{}/results/{}_x{}_patchsize{}_{}'.format(self.dir, filename, scale,size,index)
        #import pdb; pdb.set_trace()
        postfix = ('SR', 'LR', 'HR')
        for v, p in zip(save_list, postfix):
            #normalized = save_list.data.mul(255 / self.args.rgb_range)
            normalized = v.data.mul(255 / self.args.rgb_range)
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
            #misc.imsave('{}{}.png'.format(filename, p), ndarr)
